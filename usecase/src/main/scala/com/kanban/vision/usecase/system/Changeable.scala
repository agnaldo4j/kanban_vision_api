...
trait Changeable extends ChangePerformer[SystemCommand]{
  override def change[RETURN](command: SystemCommand): Try[KanbanSystemChanged[RETURN]] = {
    command match {
      case AddOrganization(name, kanbanSystem) =>
        kanbanSystem.organizationByName(name) match {
          case Success(Some(_)) => Failure(new OrganizationAlreadyExists(name))
          case Success(None) => {
            kanbanSystem
